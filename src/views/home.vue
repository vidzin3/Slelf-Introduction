<template>
    <main class="mt-10 lg:mt-24">
        <div class="w-[90%] m-auto lg:w-full">
            <div class="flex lg:flex-row flex-col px-5 py-5 bg-white rounded-xl shadow-2xl">
                <div class="w-full">
                    <img class="w-full lg:w-4/5 rounded-lg" src="../assets/img/myself.png" alt="">
                </div>
                <div class="flex flex-col lg:p-0 p-4 gap-4 lg:gap-0 lg:w-1/2">
                    <div class="w-full text-center lg:text-start lg:mt-5">
                        <p class="font-mono text-xl mb-4 text-black">HELLO EVERYBODY,</p>
                        <h1 class="text-4xl lg:text-5xl text-black font-sans font-bold my-5">I AM KOUCH DAVID</h1>
                        <p class="text-xl font-bold mb-5 text-black">JUNIOR WEB DEVELOPER</p>
                        <p class="text-xl font-sans text-gray-400">You will begin to realise why this exercise is called Nothing is impossible. ...</p>
                        <p class="text-xl font-sans text-gray-400">work in Ministry of Econamic and financial Pusat (currently worked)</p>
                    </div>
                    <div class="flex justify-center lg:justify-start items-center h-full gap-4">
                        <div class="w-10 h-10 text-center flex items-center justify-center rounded-lg text-black bg-gray-100 cursor-pointer shadow-lg hover:bg-blue-400 hover:text-white">
                            <font-awesome-icon icon="fa-brands fa-facebook-f" />
                        </div>
                        <div class="w-10 h-10 text-center flex items-center justify-center rounded-lg text-black bg-gray-100 cursor-pointer shadow-lg hover:bg-white hover:text-blue-600">
                            <a href="">
                                <font-awesome-icon icon="fa-brands fa-twitter" />
                            </a>
                        </div>
                        <div class="w-10 h-10 text-center flex items-center justify-center rounded-lg text-black bg-gray-100 cursor-pointer shadow-lg hover:bg-black hover:text-white">
                            <a href="https://github.com/vidzin3">
                                <font-awesome-icon icon="fa-brands fa-github" />
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
</template>

<script>
    export default {

    }
</script>